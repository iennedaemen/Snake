...
VS_OUTPUT VS(VS_INPUT input){
	...
	for (int i = 0; i < 4; ++i)
	{
		int currentBone = input.blendIndices[i];
		if (currentBone > -1)
		{
			transformedPosition += mul(mul(input.blendWeights[i], originalPosition), gBones[currentBone]);
			transformedNormal += mul(input.blendWeights[i], input.normal);
		}
