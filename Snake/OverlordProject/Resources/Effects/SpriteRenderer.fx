...
{
	pos = float3(pos.x-pivotOffset.x, pos.y - pivotOffset.y, pos.z);
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	//VERTEX 1 [LT]
	CreateVertex(triStream, newpos, color, texCoord, rotation, rotCosSin, offset, pivotOffset); //Change the color data too!

	//VERTEX 2 [RT]
	texCoord = float2(1, 0);
	newpos.x = position.x + gTextureSize.x * scale.x;
	CreateVertex(triStream, newpos, color, texCoord, rotation, rotCosSin, offset, pivotOffset); //Change the color data too!

	//VERTEX 3 [LB]
	texCoord = float2(0, 1);
	newpos.x = position.x;
	newpos.y = position.y + gTextureSize.y * scale.y;
	CreateVertex(triStream, newpos, color, texCoord, rotation, rotCosSin, offset, pivotOffset); //Change the color data too!
