...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	if (m_pVertexBuffer == nullptr || m_Sprites.size() > m_BufferSize)
	{
		SafeRelease(m_pVertexBuffer);
		if (m_Sprites.size() > m_BufferSize) m_BufferSize = m_Sprites.size();
		D3D11_BUFFER_DESC buffDesc;
		buffDesc.Usage = D3D11_USAGE_DYNAMIC;
		buffDesc.ByteWidth = sizeof(SpriteVertex) * m_BufferSize;
		buffDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		buffDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		buffDesc.MiscFlags = 0;

		const auto hr = gameContext.pDevice->CreateBuffer(&buffDesc, nullptr, &m_pVertexBuffer);
		if (Logger::LogHResult(hr, L"SpriteRenderer::creating new vertex buffer failed!"))
			return;
	}
